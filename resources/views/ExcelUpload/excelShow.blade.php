...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto" style="font-family: Cairo">{{ __('Burials') }}</h4><span
                    class="text-muted mt-1 tx-13 mr-2 mb-0" style="font-family: Cairo">/ {{ __('Burials List') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4">
                                ...
                                <select name="cemetery_id" id="cemetery_id" class="form-control select2">
                                    ...
                                    @foreach (App\Models\BurialExcel::select('cemetery_name')->distinct()->get() as $cemetery)
                                        <option value="{{ $cemetery->cemetery_name }}">{{ $cemetery->cemetery_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                <button type="submit" class="btn btn-success"
                                    style="margin-top: 27px;">{{ __('Search') }}</button>
                            </div>
                        </div>
                    </form>
                    <form action="{{ route('ExcelShow.bulck_delete') }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <button type="submit" class="btn btn-danger"><i class="fa fa-trash"></i>
                                    {{ __('Delete Selected Rows') }}</button>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table display" id="example">
                                <thead>
                                    <tr>
                                        <th class="wd-5p border-bottom-0 no-sort"><input type="checkbox" id="check-all"
                                                title="{{ __('Check All') }}"></th>
                                ...
                                <tbody>
                                    @foreach ($burials as $item)
                                        <tr>
                                            <td>
                                                <input type="checkbox" class="checkbox" name="burial_id[]"
                                                    value="{{ $item->id }}">
                                            </td>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ $item->name }}</td>
