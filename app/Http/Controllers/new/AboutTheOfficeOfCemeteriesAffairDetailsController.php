...
class AboutTheOfficeOfCemeteriesAffairDetailsController extends Controller
{
    ...
    public function upload(Request $request, $id)
    {
        try 
        {
            ...
            $libares = new AboutTheOfficeOfCemeteriesAffairDetails();
            if ($request->has('file')) {
                $extension = $request->file('file')->extension();
                $image_name = upload('AboutTheOfficeOfCemeteriesAffairController-details/', $extension, $request->file('file'));
            } else {
                $image_name = 'def.png';
            }
            if($request->type == 1)
        ...
        {
            return redirect()->route('AboutTheOfficeOfCemeteriesAffairController.show',$id)->with(['error' => __('There Is A Problem With The Server')]);
        }    
    }
    ...
    {        
        try 
        {
            $libares =  AboutTheOfficeOfCemeteriesAffairDetails::findOrFail($id);
            $libares->forceDelete();
